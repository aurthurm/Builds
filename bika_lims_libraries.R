...
LimsEasyRead <- function(make_readable){
	readable_data <- rename(
		...
		"Analyses.0.Result" = "Results",
		"Analyses.0.ResultCaptureDate" = "Date Result Captured",
		"Analyses.0.DateAnalysisPublished" = "Date Analysis Published",
		"Analyses.0.DateReceived" = "Date Analysis Received",
		"Analyses.0.review_state" = "Analysis State",
		"Analyses.0.Unit" = "Unit",
		"Analyses.0.creation_date" = "Date Analysis Creation"
...
LimsCombineCSV <- function(){
	# Load the data
	patients <- read.csv2("./statistics/patients.csv", header = TRUE, stringsAsFactors = FALSE)
	analysis <- read.csv("./statistics/results.csv", header = TRUE, stringsAsFactors = FALSE)
#	cases <- read.csv("./bika_csv_files/cases.csv", header = TRUE, stringsAsFactors = FALSE)

	# Renaming to obtain uniqueness using PatientUID
	library(reshape)
	patients <- rename(patients, c("UID" = "Patient_uid")) # change UID in patients to PatientUID
#	cases <- rename(cases, c("Patient_uid" = "Patient_uid")) # change Patient_uid in cases to PatientUID

	# merging the 3 csv files to one
	patients_combined <- merge(patients, analysis, by = "Patient_uid", all.x = TRUE)
	...
	write.csv(patients_combined, file = "./statistics/Patents_Combined.csv")
	...
	write.csv(patients_combined_no_NAs, file = "./statistics/Patents_Combined_no_NA.csv")
